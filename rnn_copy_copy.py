...
trainY = np.reshape(trainY, (trainX.shape[0], look_back,trainX.shape[2]))
print(trainX[-1][:][:])
...
def extend(train, last_note,look_back):
	full = np.vstack((train[0:look_back-1][:][:],last_note))
...
while True:
	...
	print(np.shape(out))
	#print(out[:][-1][:])
	out = np.reshape(out,(look_back,vocab_size))
	print(np.shape(out))
	for j in range(0,look_back):
		print(j)
		out_bin = to_categorical(np.argmax(out[j][:]),vocab_size)
		print(out_bin)
		print(np.shape(out_bin))
		data_X.append(out_bin)
	print(np.shape(np.array(data_X)))
	trainX = np.reshape(np.array(data_X), (1,look_back,vocab_size))
	print(np.shape(trainX))
	#pred =np.argmax(out)
	#print(pred)
	#print(pred)
